...
class SphereTest {
    ...
    void testFindIntersections() {
        ...
        final var result1 = sphere.findIntersections(new Ray(pm100, v310));//.stream().sorted(Comparator.comparingDouble(p) -> p.distance(p01))).toList();
        assertEquals(2, result1.size(), "Wrong number of points");
        assertEquals(exp1, result1, "Ray crosses sphere");
        // TC03: Ray starts inside the sphere (1 point)
        final var exp2 = List.of(p000);
        final var result2 = sphere.findIntersections(new Ray(p0p500, vm100));//.stream().sorted(Comparator.comparingDouble(p) -> p.distance(p01))).toList();
        ...
        final var result4 = sphere.findIntersections(new Ray(pm100, v100));//.stream().sorted(Comparator.comparingDouble(p) -> p.distance(p01))).toList();
        ...
        final var result5 = sphere.findIntersections(new Ray(p101, v00m1));//.stream().sorted(Comparator.comparingDouble(p) -> p.distance(p01))).toList();
        assertEquals(1, result5.size(), "Wrong number of points");
        assertEquals(exp5, result5, "Ray crosses sphere");
        // TC15: Ray starts inside (1 points)
        final var result6 = sphere.findIntersections(new Ray(p100p5, v00m1));//.stream().sorted(Comparator.comparingDouble(p) -> p.distance(p01))).toList();
        assertEquals(1, result6.size(), "Wrong number of points");
        assertEquals(exp5, result6, "Ray crosses sphere");
        // TC16: Ray starts at the center (1 points)
        final var result7 = sphere.findIntersections(new Ray(p100, v00m1));//.stream().sorted(Comparator.comparingDouble(p) -> p.distance(p01))).toList();
